...
const App:FC = () => {
    ...
    const initializeSession = async () => {
      ...
      socket.on('transcription', (responsePhrase) => {
        ...
        setTranscription(function (previousTranscription) {
          if (previousTranscription === null) {
            throw new Error('Transcription received for non-existent transcription');
          }
          const transcription = {...previousTranscription};
          ...
          if (!transcription.phrases[responsePhrase.index]) {
            ...
            transcription.phrases[responsePhrase.index].incomingTranscription = responsePhrase.transcription;
        ...
        setTimeout(() => {
          setTranscription(function (previousTranscription) {
            if (previousTranscription === null) { return null; }
            const transcription = {...previousTranscription};
            transcription.phrases[responsePhrase.index] = {
              ...transcription.phrases[responsePhrase.index],
  ...
  const startRecording = async () => {
    ...
    initializeNewRecording().then((response) => {
    ...
    <div className="max-w-prose mx-auto px-4 font-serif text-xl py-20">
      <div>
        <div className="text-gray-900">
          {recording && transcription && transcription.phrases && (transcription.phrases.length == 0) && <span className="animate-pulse-fast">|</span>}
          {transcription && transcription.phrases && transcription.phrases.map((phrase, index) => (
